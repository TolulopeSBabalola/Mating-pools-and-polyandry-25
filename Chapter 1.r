...
ggplot(data=combo,aes(x=means,fill=sex, color = sex))+
  ...
  theme(axis.title= element_text(color="black",family = "Arial", size=12,face="bold"),
        legend.title =  element_text(color="black",family = "Arial", size=12,face="bold"),
...
Surv(time=Lekreturnrates$SwarmsBetween, event= Lekreturnrates$Status)[1:10]

Y=Surv(time=Lekreturnrates$SwarmsBetween, event= Lekreturnrates$Status)

kmfig=survfit(Y ~ Lekreturnrates$Sex)


plot(kmfig, lty = c("solid", "dashed"), 
     col = c("black", "grey"), xlab = "Survival Time In Days", 
     ylab = "Survival Probabilities")

model_fit <- survfit(Surv(SwarmsBetween, Status) ~ Sex, data = Lekreturnrates)


autoplot(model_fit) + 
  labs(x = "\n Time Out (Swarms)", y = "Survival Probabilities \n",fill = "sex", color= "sex") + 
  theme_classic()+
  scale_color_manual(values = c("#5A032A","#B05300"))+
